...
public class ProgressController {
    ...
    public ResponseEntity<ProgressDto> addRecipeToProgress(
            Authentication authentication,
            @RequestParam Long recipeId,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date) {
        LocalDate actualDate = date != null ? date : LocalDate.now();
        return ResponseEntity.ok(progressService.addRecipeToProgress(authentication.getName(), recipeId, actualDate));
    }
}
