...
export default function AdminPage() {
  ...
  const [newUserForm, setNewUserForm] = useState({ username: '', email: '', password: '', role: 'ROLE_USER' });
  ...
  const [recipeForm, setRecipeForm] = useState<Partial<RecipeDto>>({});
  ...
  const createUser = async () => {
    try {
      ...
      setNewUserForm({ username: '', email: '', password: '', role: 'ROLE_USER' });
      loadTab();
    } catch (e: any) {
      toast.error(e.response?.data?.message || 'Ошибка создания пользователя');
    }
  };
  ...
  const moderateRecipe = async (id: number, approve: boolean) => {
    try {
      if (approve && editingRecipe) {
        // Если одобряем с редактированием
        await api.put(`/recipes/${id}`, recipeForm);
      }
  ...
  const startEditRecipe = (r: RecipeDto) => {
    setEditingRecipe(r);
    setRecipeForm({ ...r });
  };
    ...
    <div className="space-y-6">
      ...
      {tab === 'users' && (
        <div className="space-y-4">
          ...
          {showAddUser && (
            <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm ring-1 ring-gray-200 dark:ring-slate-800 animate-fade-in">
              <h3 className="text-lg font-semibold mb-4 dark:text-slate-100">Новый пользователь</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                  <label className="text-xs text-gray-500 dark:text-slate-400 mb-1 block">Имя пользователя</label>
                  <input
                    className="block w-full px-3 py-2 border dark:border-slate-700 rounded text-sm bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 outline-none"
                ...
                <div>
                  <label className="text-xs text-gray-500 dark:text-slate-400 mb-1 block">Пароль</label>
                  <input
                    type="password"
                    className="block w-full px-3 py-2 border dark:border-slate-700 rounded text-sm bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 outline-none"
              ...
              <div className="flex justify-end mt-4">
                <button 
                  onClick={createUser}
                  className="bg-olive text-white px-6 py-2 rounded text-sm hover:bg-olive-dark transition-colors"
                ...
                <div className="space-y-4 animate-fade-in">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="md:col-span-2">
                      ...
                      <input 
                        className="w-full px-3 py-1.5 border dark:border-slate-700 rounded bg-white dark:bg-slate-800" 
                  ...
                  <div className="flex gap-2 justify-end">
                    <button onClick={() => setEditingRecipe(null)} className="px-3 py-1 text-sm bg-gray-100 dark:bg-slate-800 rounded">Отмена</button>
                    <button onClick={() => moderateRecipe(r.id, true)} className="px-3 py-1 text-sm bg-olive text-white rounded font-medium">Одобрить и Сохранить</button>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  <div className="flex gap-2 mt-2">
                    <button onClick={() => startEditRecipe(r)} className="bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-300 px-3 py-1 rounded text-sm hover:bg-gray-200">Редактировать</button>
      ...
      {tab === 'ingredients' && <CrudPanel<Ingredient> items={ingredients} name="ингредиент" fields={['name', 'unit', 'calories', 'proteins', 'fats', 'carbohydrates']} apiPath="/admin/ingredients" reload={loadTab} />}
